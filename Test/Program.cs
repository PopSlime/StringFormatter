...
namespace Test
{
    ...
    class Program
    {
        ...
        {
            var f = new StringByteBuffer();
            RunTest(f);
        }

        private static void RunTest(StringByteBuffer f)
        {
            ...
            StringByteBuffer.SetCustomFormatter<Blah>(CustomFormat);
            f.Clear();
        ...
        static void PerfTest()
        {
            ...
            var byteFormatter = new StringByteBuffer();
            ...
            timer.Stop();
            Console.WriteLine("Mine (byte) : {0} us/format", timer.ElapsedMilliseconds * 1000.0 / (count * mul));
            Console.WriteLine("GCs  : {0}", GC.CollectionCount(0) - gcCount);
            Console.WriteLine();
        ...
        static void CustomFormat(BaseStringBuffer buffer, Blah blah, StringView format)
        {
            if (format == "yes")
                buffer.Append("World!");
            else
