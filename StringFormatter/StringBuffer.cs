...
namespace System.Text.Formatting {
    ...
    public unsafe sealed partial class StringBuffer
    {
        ...
        public StringBuffer(int capacity, Encoding encoding = null)
        {
        ...
        public int CopyTo(byte* dest, int sourceIndex, int count, int byteCount, Encoding encoding)
        {
            if (count < 0)
                throw new ArgumentOutOfRangeException(nameof(count));
            if (sourceIndex + count > currentCharCount || sourceIndex < 0)
        ...
        bool AppendSegment<T>(ref char* currRef, char* end, byte* dest, ref int prevArgIndex, ref T args) where T : IArgSet
        {
            var curr = currRef;
            var c = '\x0';
            while (curr < end)
